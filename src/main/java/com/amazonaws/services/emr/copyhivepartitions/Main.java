  ...
  public static void main(String args[]) {
    ...
    try {
      ...
      opt.addOption(
          ...
          "example",
          false,
          "java -jar copyhivepartitions-0.0.1-SNAPSHOT.jar  -c jdbc:mysql://emrdb.cz8lzaalmbmy.us-east-1.rds.amazonaws.com:3306/hive -u root -p password -s my_old_table -t my_new_table  -o insert.sql");
      opt.addOption(OptionBuilder
          .withArgName("database.url")
          .withDescription(
              "the mysql connection string. e.g [jdbc:mysql://emrdb.cz8lzaalmbmy.us-east-1.rds.amazonaws.com:3306/hive]")
          .hasArg().isRequired(true).create("c"));

      opt.addOption(OptionBuilder.withArgName("database.username")
          .withDescription("the username for connecting to the database").hasArg().isRequired(true).create("u"));

      opt.addOption(OptionBuilder.withArgName("database.password").withDescription("the password for the mysql user")
          .hasArg().isRequired(true).create("p"));
      opt.addOption(OptionBuilder.withArgName("sourceTableName")
          .withDescription("The name of the source table that the partitions will be copied from").hasArg()
          .isRequired(true).create("s"));
      opt.addOption(OptionBuilder.withArgName("targetTableName")
          .withDescription("The name of the destination table that the partitions will be copied into").hasArg()
          .isRequired(true).create("t"));
      opt.addOption(OptionBuilder.withArgName("output.file").withDescription("The name of the output file").hasArg()
